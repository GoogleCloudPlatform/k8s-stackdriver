...
// EventOwnerChecker determines whether this pod should process a given event.
// Used for hash-based partitioning across multiple pods.
type EventOwnerChecker interface {
	IsOwner(eventKey string) bool
}
...
type sdSink struct {
	...
	// ownerChecker determines event ownership for hash-based partitioning.
	// When nil, this pod processes all events (single-pod mode).
	ownerChecker EventOwnerChecker
}

func newSdSink(writer sdWriter, clock clock.Clock, config *sdSinkConfig, factory *monitoredResourceFactory, podLabelCollector podlabels.PodLabelCollector, ownerChecker EventOwnerChecker) *sdSink {
	return &sdSink{
...
func (s *sdSink) OnAdd(event *corev1.Event) {
	...
	if s.ownerChecker != nil {
		eventKey := event.Namespace + "/" + event.Name
		if !s.ownerChecker.IsOwner(eventKey) {
			droppedByHashCount.Inc()
			return
		}
	}
	ownedEntryCount.Inc()
...
func (s *sdSink) OnUpdate(_ *corev1.Event, newEvent *corev1.Event) {
	...
	if s.ownerChecker != nil {
		eventKey := newEvent.Namespace + "/" + newEvent.Name
		if !s.ownerChecker.IsOwner(eventKey) {
			droppedByHashCount.Inc()
			return
		}
...
func (s *sdSink) Run(stopCh <-chan struct{}) {
	...
	// Periodically report queue depth for HPA metrics.
	...
	for {
		...
		case <-s.getTimerChannel():
			...
			break
		case <-queueTicker.C:
			queueDepth.Set(float64(len(s.logEntryChannel)))
			break
		case <-stopCh:
