...
func (w sdWriterImpl) Write(entries []*sd.LogEntry, logName string, resource *sd.MonitoredResource) {
	...
	// We retry forever with exponential backoff, until request either
	// succeeds or API returns BadRequest, which means that the request is
	// malformed, e.g. because it contains too large entries. This behavior
	// mirrors the way logging agent pushes logs to Stackdriver.
	...
	for {
		...
		glog.Warningf("Failed to send request to Stackdriver, retrying in %v: %v", retryDelay, err)
		time.Sleep(retryDelay)
		retryDelay = nextRetryDelay(retryDelay)
	}
}

// nextRetryDelay computes the next retry delay using exponential backoff with jitter.
func nextRetryDelay(current time.Duration) time.Duration {
	next := time.Duration(float64(current) * retryMultiplier)
	if next > maxRetryDelay {
		next = maxRetryDelay
	}
	// Add jitter: +-20% of the delay.
	jitterRange := float64(next) * retryJitter
	jittered := float64(next) + (rand.Float64()*2-1)*jitterRange
	return time.Duration(math.Max(float64(initialRetryDelay), jittered))
}
