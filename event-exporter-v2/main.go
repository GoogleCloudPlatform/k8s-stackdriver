...
import (
	...
	"github.com/GoogleCloudPlatform/k8s-stackdriver/event-exporter-v2/kubernetes/watchers"
	"github.com/GoogleCloudPlatform/k8s-stackdriver/event-exporter-v2/peerdiscovery"
	"github.com/GoogleCloudPlatform/k8s-stackdriver/event-exporter-v2/sinks/stackdriver"
...
var (
	...
	headlessServiceName          = flag.String("headless-service-name", "", "Name of the headless Service for peer discovery in HA mode. Empty means single-pod mode (no partitioning).")
)
...
func main() {
	...
	// Set up peer discovery for HA mode if headless service name is provided.
	var pd *peerdiscovery.PeerDiscovery
	var ownerChecker stackdriver.EventOwnerChecker
	if *headlessServiceName != "" {
		podIP := os.Getenv("POD_IP")
		podNamespace := os.Getenv("POD_NAMESPACE")
		if podIP == "" || podNamespace == "" {
			glog.Fatalf("POD_IP and POD_NAMESPACE environment variables are required for HA mode. "+
				"Set them via the Downward API in your pod spec. POD_IP=%q, POD_NAMESPACE=%q", podIP, podNamespace)
		}
		pd = peerdiscovery.New(client, *headlessServiceName, podNamespace, podIP)
		ownerChecker = pd
		glog.Infof("HA mode enabled: headless-service=%s, pod-ip=%s, namespace=%s",
			*headlessServiceName, podIP, podNamespace)
	}

	sinkFactory := stackdriver.NewSdSinkFactory()
	sink, err := sinkFactory.CreateNewWithOwnerChecker(strings.Split(*sinkOpts, " "), informer, ownerChecker)
	if err != nil {
	...
	eventExporter := newEventExporter(client, sink, *resyncPeriod, parsedLabelSelector, *listerWatcherOptionsLimit, *listerWatcherEnableStreaming, st, pd)

	// Expose the Prometheus http endpoint and readiness probe.
	go func() {
		http.Handle("/metrics", promhttp.Handler())
		http.HandleFunc("/readyz", func(w http.ResponseWriter, r *http.Request) {
			if pd != nil && !pd.Ready() {
				w.WriteHeader(http.StatusServiceUnavailable)
				fmt.Fprint(w, "not ready: waiting for peer discovery")
				return
			}
	...
	// If in HA mode, wait briefly for peer discovery to sync before starting
	// the event watcher. This reduces the startup grace period window.
	if pd != nil {
		glog.Info("Waiting for peer discovery to sync...")
		if pd.WaitForSync(30 * time.Second) {
			glog.Infof("Peer discovery synced: %d peers", pd.PeerCount())
		} else {
			glog.Warning("Peer discovery sync timed out, starting in grace period mode (processing all events)")
		}
	}
